...
async function fetchOffers(page = null) {
  let indexes;
  if (! page) {
    indexes = _.range(240, 250);
  } else {
    indexes = _.range(page, page + 30);
  }

  const results = await Promise.all(indexes.map(fetchOffer));
  const offers = results.map(result => parseOffers(result.data.toString(), result.request.res.responseUrl)).filter(e => ! _.isEmpty(e));
...
function parseOffers(offers, requestURL) {
  ...
  return results.filter(e => ! _.isEmpty(e))
